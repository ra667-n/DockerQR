...
repository as the base image
FROM python:3.12-slim-bullseye

# Set the working directory to /app in the container
WORKDIR /app

# Create a non-root user named 'myuser' with a home 
directory
RUN useradd -m myuser

# Copy the requirements.txt file to the container to install 
Python dependencies
...
# Create the logs and qr_codes directories and set ownership 
for the non-root user
RUN mkdir -p /app/logs /app/qr_codes && chown myuser:myuser 
